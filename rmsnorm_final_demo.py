...
class NonDeterministicParallelRMSNorm(nn.Module):
    """非确定性并行RMSNorm - 真正模拟非确定性行为"""
    ...
    def forward(self, x):
        ...
        # 关键：当batch size < 并行度时，模拟非确定性的并行归约
...
class RMSNormFinalDemo:
    ...
    def __init__(self, device: str = 'auto', parallel_degree: int = 4, dim: int = 512):
        ...
        # 创建两种不同的RMSNorm实现
        self.non_deterministic_norm = NonDeterministicParallelRMSNorm(dim, parallel_degree=parallel_degree).to(self.device)
    ...
    def test_batch_invariance(self, batch_sizes: List[int] = [1, 4, 8], num_trials: int = 10) -> Dict[str, Any]:
        """测试batch invariance"""
        print("=" * 80)
        print("RMSNorm最终Batch Invariance测试")
        print("=" * 80)
...
def main():
    ...
    # 创建演示实例
    demo = RMSNormFinalDemo(parallel_degree=4, dim=512)
